...
Extracted from [HIREDIS README.md](../hiredis/README.md)
...
- Uses "high level printf-like API" supporting the sending of commands and receiving of replies
- It comes with a reply parser that is decoupled from the I/O layer. It
...
HIREDIS uses a `redisContext*` to keep state
...
The function `redisConnect` creates `redisContext` struct.
    - holds state for a connection.
    - `err` is non-zero for defective connections.
    - `errstr` - string with a description of the error.
