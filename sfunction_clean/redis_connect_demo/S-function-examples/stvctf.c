...
static void mdlInitializeSizes(SimStruct *S)
{
     ...
     *     coefficients
     */
    nCoeffs     = (int_T)mxGetNumberOfElements(DEN(S));
...
static void mdlInitializeConditions(SimStruct *S)
{
     ...
     */
    for (i = 0; i < (size_t)nContStates; i++) {
        x0[i]       = 0.0;
        numBank0[i] = 0.0;
        denBank0[i] = 0.0;
    }
     ...
     */
    {
        ...
        size_t       numParamLen = mxGetNumberOfElements(NUM(S));

        const real_T *denParam   = mxGetPr(DEN(S));
        size_t       denParamLen = mxGetNumberOfElements(DEN(S));
...
static void mdlUpdate(SimStruct *S, int_T tid)
{
    ...
    if (ssIsSampleHit(S, 1, tid)) {
        ...
        if (allZero) { /* if numerator is all zero */
            const real_T *numParam   = mxGetPr(NUM(S));
            int_T        numParamLen = (int_T)mxGetNumberOfElements(NUM(S));
            /*
        ...
        if (allZero) {  /* If denominator is all zero. */
            const real_T *denParam   = mxGetPr(DEN(S));
            int_T        denParamLen = (int_T)mxGetNumberOfElements(DEN(S));
